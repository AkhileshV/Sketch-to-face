...
class Coach:
	def __init__(self, opts):
		...
		# load pretrained network and freeze initial layers for transfer learning
		ct = 0
		...
		print("layer count:", ct)

		arch = summary(self.net, torch.rand((1, 1, 256, 256)).to(self.device)) # (3, 256, 256) if label_nc=0 and (1, 256, 256) if label_nc=1
	...
	def train(self):
		...
		while self.global_step < self.opts.max_steps:
			for batch_idx, batch in enumerate(self.train_dataloader):
				...
				# Scales loss.  Calls backward() on scaled loss to create scaled gradients. This avoids gradients from becoming 0
